...
App::patch('/v1/databases/:databaseId/collections/:collectionId/documents/:documentId')
    ...
    ->action(function (string $databaseId, string $collectionId, string $documentId, string|array $data, ?array $permissions, ?\DateTime $requestTimestamp, Response $response, Database $dbForProject, Event $events, string $mode) {
        ...
        try {
            $databaseCollectionId = 'database_' . $database->getInternalId() . '_collection_' . $collection->getInternalId();
            if ($documentSecurity && !$valid) {
                $document = $dbForProject->withRequestTimestamp(
                    $requestTimestamp,
                    function () use ($dbForProject, $databaseCollectionId, $document, $data) {
                ...
                $document = Authorization::skip(function () use ($dbForProject, $requestTimestamp, $databaseCollectionId, $document, $data) {
...
App::delete('/v1/databases/:databaseId/collections/:collectionId/documents/:documentId')
    ...
    ->action(function (string $databaseId, string $collectionId, string $documentId, ?\DateTime $requestTimestamp, Response $response, Database $dbForProject, Event $events, Delete $deletes, string $mode) {
        ...
        if ($documentSecurity && !$valid) {
            try {
                $dbForProject->withRequestTimestamp($requestTimestamp, function () use ($dbForProject, $databaseCollectionId, $documentId) {
                    return $dbForProject->deleteDocument($databaseCollectionId, $documentId);
                });
            } catch (AuthorizationException) {
                throw new Exception(Exception::USER_UNAUTHORIZED);
            }
        } else {
            Authorization::skip(function () use ($dbForProject, $requestTimestamp, $databaseCollectionId, $documentId) {
                return $dbForProject->withRequestTimestamp($requestTimestamp, function () use ($dbForProject, $databaseCollectionId, $documentId) {
                    return $dbForProject->deleteDocument($databaseCollectionId, $documentId);
                });
            });
        }

        $dbForProject->deleteCachedDocument($databaseCollectionId, $documentId);
